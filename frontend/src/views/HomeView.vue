<template>
    <main>
        <section>
            ...
            <FileUpload name="demo[]" url="http://localhost:3000/upload" customUpload @uploader="uploadFile($event)"
                :multiple="false" accept="application/pdf" :maxFileSize="1000000">
                <template #thumbnail>
        ...
        <section class="exisiting-statements" style="margin-top: 32px;">
            ...
            <DataTable v-model:editingRows="editingRows" :value="rawStatements" editMode="row" dataKey="id"
                ...
                <Column field="month" header="Month" style="width: 20%">
                    <template #editor="{ data, field }">
                        <Dropdown v-model="data[field]" :options="availableMonths" optionLabel="label" optionValue="value"
                ...
                <Column field="year" header="Year" style="width: 20%">
                    <template #editor="{ data, field }">
                        <Dropdown v-model="data[field]" :options="availableYears" optionLabel="label" optionValue="value"
                            placeholder="Select a Year" class="w-full md:w-14rem" />
                    </template>
                    <template #body="slotProps">
                ...
                <Column field="bank" header="Bank" style="width: 20%">
                    <template #editor="{ data, field }">
                        <Dropdown v-model="data[field]" :options="availableBanks" optionLabel="label" optionValue="value"
                            placeholder="Select a Bank" class="w-full md:w-14rem" />
                    </template>
                    <template #body="slotProps">
                ...
                <Column v-if="!isRowEditorMode" header="Actions" bodyStyle="text-align:center">
                    <template #body="slotProps">
                        <div style="display: flex; gap: 8px;">
                            <Button :disabled="!canTriggerParse(slotProps.data)" icon="pi pi-play" aria-label="Trigger"
                                size="small" @click="triggerParseStatement(slotProps.data.id)" />
                            <Button icon="pi pi-trash" aria-label="Trigger" size="small" severity="danger"
                                @click="triggerTrashStatement(slotProps.data.id)" />
                        </div>
                    </template>
                </Column>
        ...
        <section class="exisiting-statements" style="margin-top: 32px;">
            ...
            <DataTable v-model="rawReports" :value="rawReports" dataKey="id" tableClass="editable-cells-table"
                ...
                <Column header="Actions" bodyStyle="text-align:center" style="width: 20%">
                    <template #body="slotProps">
                        <div style="display: flex; gap: 8px;">
                            <Button icon="pi pi-eye" aria-label="Trigger" size="small" severity="primary"
                                @click="viewSingleReport(slotProps.data.id)" />
                        </div>
                    </template>
                </Column>
            </DataTable>
...
export default {
    ...
    methods: {
        ...
        viewSingleReport(rawReportId) {
            this.$router.push({ name: 'RawReport', params: { rid: rawReportId } })
        }
    }
};
</script>